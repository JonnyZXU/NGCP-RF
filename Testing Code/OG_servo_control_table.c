...
typedef struct {
    ...
    double min_us;
    double max_us;
    cal_point_t cal[MAX_CAL_POINTS];
    int cal_count;
} servo_t;

static void servo_sort_cal(servo_t *s) {
    for (int i = 0; i < s->cal_count - 1; i++) {
        for (int j = i + 1; j < s->cal_count; j++) {
            if (s->cal[j].deg < s->cal[i].deg) {
                cal_point_t tmp = s->cal[i];
                s->cal[i] = s->cal[j];
                s->cal[j] = tmp;
            }
        }
...
static int servo_validate_cal(const servo_t *s) {
    if (s->cal_count < 2) return -1;
    for (int i = 0; i < s->cal_count - 1; i++) {
        if (s->cal[i + 1].deg <= s->cal[i].deg) return -1;
    }
    return 0;
}

static int servo_set_cal_point(servo_t *s, double deg, double pulse_us) {
    for (int i = 0; i < s->cal_count; i++) {
        if (fabs(s->cal[i].deg - deg) < 1e-9) {
            s->cal[i].pulse_us = pulse_us;
            servo_sort_cal(s);
            return servo_validate_cal(s);
        }
    }

    if (s->cal_count >= MAX_CAL_POINTS) return -1;

    s->cal[s->cal_count].deg = deg;
    s->cal[s->cal_count].pulse_us = pulse_us;
    s->cal_count++;
    servo_sort_cal(s);
    return servo_validate_cal(s);
}

static void servo_print_cal(const servo_t *s) {
    puts("Calibration table:");
    for (int i = 0; i < s->cal_count; i++) {
        printf("  %2d: deg=%8.3f  pulse_us=%8.3f\n",
               i, s->cal[i].deg, s->cal[i].pulse_us);
    }
}

static double servo_deg_to_pulse_us(const servo_t *s, double deg) {
    if (s->cal_count < 2) return s->center_us;

    if (deg <= s->cal[0].deg) return s->cal[0].pulse_us;
    if (deg >= s->cal[s->cal_count - 1].deg) return s->cal[s->cal_count - 1].pulse_us;

    for (int i = 0; i < s->cal_count - 1; i++) {
        double d0 = s->cal[i].deg;
        double d1 = s->cal[i + 1].deg;

        if (deg >= d0 && deg <= d1) {
            double frac = (deg - d0) / (d1 - d0);
            return s->cal[i].pulse_us + frac * (s->cal[i + 1].pulse_us - s->cal[i].pulse_us);
        }
    }

    return s->cal[s->cal_count - 1].pulse_us;
}

static double servo_pulse_to_deg(const servo_t *s, double pulse_us) {
    if (s->cal_count < 2) return 0.0;

    int increasing = (s->cal[s->cal_count - 1].pulse_us >= s->cal[0].pulse_us);

    if (increasing) {
        if (pulse_us <= s->cal[0].pulse_us) return s->cal[0].deg;
        if (pulse_us >= s->cal[s->cal_count - 1].pulse_us) return s->cal[s->cal_count - 1].deg;

        for (int i = 0; i < s->cal_count - 1; i++) {
            double p0 = s->cal[i].pulse_us;
            double p1 = s->cal[i + 1].pulse_us;
            if (pulse_us >= p0 && pulse_us <= p1) {
                double frac = (pulse_us - p0) / (p1 - p0);
                return s->cal[i].deg + frac * (s->cal[i + 1].deg - s->cal[i].deg);
            }
        }
    } else {
        if (pulse_us >= s->cal[0].pulse_us) return s->cal[0].deg;
    ...
    return s->cal[s->cal_count - 1].deg;
}
...
static void help(void) {
    ...
    puts("  status");
    puts("  cal show");
    puts("  cal set <deg> <pulse_us>    (add/update calibration point)");
...
int main(void) {
    ...
    if (servo_validate_cal(&s) < 0) {
        fprintf(stderr, "Invalid calibration table.\n");
        return 1;
    }

    if (servo_set_pulse_us(&s, servo_deg_to_pulse_us(&s, 0.0)) < 0) {
    ...
    while (!exiting) {
        ...
        if (!strcmp(cmd, "cal")) {
            ...
            if (sscanf(p + 3, "%31s", sub) == 1) {
                if (!strcmp(sub, "show")) {
                    ...
                    double deg, pulse_us;
                    if (sscanf(p + 3, "%*s %lf %lf", &deg, &pulse_us) == 2) {
                        if (pulse_us < s.min_us || pulse_us > s.max_us) {
                            printf("Pulse %.2f is outside clamp range [%.0f..%.0f]\n",
                                   pulse_us, s.min_us, s.max_us);
                        } else if (servo_set_cal_point(&s, deg, pulse_us) < 0) {
                            puts("Failed to add/update calibration point.");
                        } else {
                            servo_recalc_deg_from_pulse(&s);
                            printf("Calibration point set: deg=%.3f pulse_us=%.3f\n", deg, pulse_us);
                        }
                    } else {
                        puts("Usage: cal set <deg> <pulse_us>");
                    }
                } else {
                    puts("Usage: cal show | cal set <deg> <pulse_us>");
                }
            } else {
                puts("Usage: cal show | cal set <deg> <pulse_us>");
            }
        ...
        if (!strcmp(cmd, "move")) {
            ...
            if (n >= 2) {
                ...
                double target = servo_deg_to_pulse_us(&s, target_deg);

                printf("Move %.2f deg from %.2f -> target %.2f deg -> pulse %.2f us\n",
                       deg, s.deg_pos, target_deg, target);
