// servo_control.c
// Uses sysfs PWM: /sys/class/pwm/pwmchip0/pwm2 (your working setup)
// Uses "Travel per Âµsec" (deg_per_us) to convert degrees <-> pulse width.
// This is the right way to get consistent "45 degrees" behavior.
//
// Build:
...
//   echo 2 | sudo tee /sys/class/pwm/pwmchip0/export >/dev/null 2>&1 || true
//
// Run:
//   sudo ./servo_control
//
...
static int set_pwm_duty_ns(long duty_ns) {
    // clamp to 0..period-1 so kernel doesn't reject it
...
// sysfs GPIO sensor read: /sys/class/gpio/gpioN/value
...
// ----- Servo model (based on your spec) -----
// We treat "mechanical degrees" as real degrees of rotation (0..360*rotations).
// Pulse is computed by a linear model around center:
//   pulse_us = center_us + (deg / deg_per_us)
//
// IMPORTANT: deg_per_us may not be exactly 0.90 for your unit. Use `scale` to calibrate.
typedef struct {
    double deg_per_us;     // default 0.90 (deg / microsecond)
    double center_us;      // usually 1500
    double pulse_us;       // current commanded pulse
    double deg_pos;        // current "mechanical" deg estimate (based on pulse model)
    double min_us;         // clamp (e.g. 500)
    double max_us;         // clamp (e.g. 2500)
} servo_t;

static void servo_recalc_deg_from_pulse(servo_t *s) {
    s->deg_pos = (s->pulse_us - s->center_us) * s->deg_per_us;
}

// Convert target mechanical degrees to pulse
static double servo_deg_to_pulse_us(const servo_t *s, double deg) {
    // deg_per_us = deg/us => us per deg = 1/deg_per_us
    double delta_us = deg / s->deg_per_us;
    return s->center_us + delta_us;
}
...
// Move smoothly from current pulse to target pulse over duration, with sensor/stop polling.
...
static int servo_hold(servo_t *s, double hold_s, int sensor_gpio) {
...
static void help(void) {
    puts("Commands:");
    puts("  status");
    puts("  scale <deg_per_us>          (default 0.90; calibrate here)");
    puts("  pulse <us>                  (direct pulse command)");
    puts("  move <deg> <sec> [hold_s]   (relative move, ex: move 45 2 1)");
    puts("  goto <deg> <sec> [hold_s]   (absolute mechanical deg, based on model)");
...
int main(void) {
    ...
    if (ensure_pwm() < 0) {
        fprintf(stderr, "PWM init failed. Make sure PWM2 exists and GPIO18 set to PWM (sudo pinctrl set 18 a3).\n");
        return 1;
    }

    servo_t s = {
        .deg_per_us = 0.90,     // from your spec
        .center_us  = 1500.0,
        .pulse_us   = 1500.0,
        .deg_pos    = 0.0,
        .min_us     = 500.0,    // Max PWM Range 500-2500us
        .max_us     = 2500.0
    };

    // Start centered
    if (servo_set_pulse_us(&s, s.center_us) < 0) {
    ...
    while (!exiting) {
        ...
        if (!strcmp(cmd, "status")) {
            printf("pulse_us=%.2f  deg_pos(model)=%.2f  deg_per_us=%.4f  center_us=%.1f  clamp=[%.0f..%.0f]  sensor=%d\n",
                   s.pulse_us, s.deg_pos, s.deg_per_us, s.center_us, s.min_us, s.max_us, sensor_gpio);
            continue;
        }

        if (!strcmp(cmd, "scale")) {
            double v;
            if (sscanf(p + 5, "%lf", &v) == 1 && v > 0.05 && v < 5.0) {
                s.deg_per_us = v;
                servo_recalc_deg_from_pulse(&s);
                printf("deg_per_us set to %.4f\n", s.deg_per_us);
            } else {
                puts("Usage: scale <deg_per_us>  (example: scale 0.75)");
            }
            continue;
        }
        ...
        if (!strcmp(cmd, "move")) {
            ...
            if (n >= 2) {
                ...
                // relative move: compute target pulse by delta
                double delta_us = deg / s.deg_per_us;
                double target = s.pulse_us + delta_us;

                printf("Move %.2f deg -> delta %.2f us -> target %.2f us\n", deg, delta_us, target);

                int rc = servo_ramp_to_pulse(&s, target, sec, sensor_gpio, 20);
                if (rc == 1) puts("Stopped by sensor.");
                else if (rc < 0) puts("Stopped.");
                else {
                    int hc = servo_hold(&s, hold_s, sensor_gpio);
        ...
        if (!strcmp(cmd, "goto")) {
            ...
            if (n >= 2) {
                stop_requested = 0;
                double target = servo_deg_to_pulse_us(&s, deg);
                printf("Goto %.2f deg -> target pulse %.2f us\n", deg, target);
        ...
        if (!strcmp(cmd, "range") || !strcmp(cmd, "search")) {
            ...
            if ((is_search && n == 3) || (!is_search && n >= 3)) {
                stop_requested = 0;
                double pa = servo_deg_to_pulse_us(&s, a);
                double pb = servo_deg_to_pulse_us(&s, b);
        ...
        if (!strcmp(cmd, "auto")) {
            ...
            // allocate motion time per step, with a hold at each milestone
            ...
            // start at 0 deg
            double cur_deg = 0.0;
            int rc = servo_ramp_to_pulse(&s, servo_deg_to_pulse_us(&s, 0.0), 0.5, sensor_gpio, 20);
            if (rc == 1) { puts("Sensor triggered."); continue; }
            if (rc < 0) { puts("Stopped."); continue; }

            for (int k = 0; k <= num_steps && !stop_requested; k++) {
                double target_deg = k * (double)step;
                // keep it within 0..360 for the milestone display, but the pulse model is linear anyway
                double shown = fmod(target_deg, 360.0);
                if (shown < 0.0) shown += 360.0;

                double target_pulse = servo_deg_to_pulse_us(&s, shown);
                ...
                rc = servo_hold(&s, hold_s, sensor_gpio);
